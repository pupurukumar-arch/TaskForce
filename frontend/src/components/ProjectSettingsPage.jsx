...
export function ProjectSettingsPage({ user }) {
  ...
  const deleteProject = async () => {
    if (
      !window.confirm(
        "Delete this project and all of its tasks, notes, and members? This cannot be undone.",
      )
    )
      return;
  ...
  return (
    <AppLayout user={user}>
      <Link
        ...
        className="inline-flex rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 shadow-sm transition hover:bg-slate-50"
      >
        Back to project
      </Link>
      <div className="mt-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Manage the project name, description, and deletion settings.
        </p>
      </div>
      ...
      {isLoading && (
        <p className="mt-6 text-sm text-slate-600">Loading project settings…</p>
      )}
        ...
        <>
          <section className="mt-6 max-w-2xl rounded-2xl border border-slate-200 bg-[#fffdf9] p-6 shadow-sm shadow-slate-200/40">
            ...
            <p className="mt-2 text-sm text-slate-500">
              This permanently removes its tasks, notes, and member records.
            </p>
            <button
